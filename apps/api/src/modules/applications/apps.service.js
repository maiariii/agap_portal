...
export async function getHydratedApplications(vacancyId = null, region = null, division = null) {
  ...
  return rows.map(row => {
    ...
    return {
      ...
      dateApplied: row.date_applied ? (typeof row.date_applied === 'string' ? row.date_applied.slice(0,10) : `${new Date(row.date_applied).getFullYear()}-${String(new Date(row.date_applied).getMonth() + 1).padStart(2, '0')}-${String(new Date(row.date_applied).getDate()).padStart(2, '0')}`) : "",
      deadline: row.vacancy_posting_end ? (typeof row.vacancy_posting_end === 'string' ? row.vacancy_posting_end.slice(0,10) : `${new Date(row.vacancy_posting_end).getFullYear()}-${String(new Date(row.vacancy_posting_end).getMonth() + 1).padStart(2, '0')}-${String(new Date(row.vacancy_posting_end).getDate()).padStart(2, '0')}`) : "",
      bachelorDegree: bachelorDegree,
      yearsExperience: Number(row.applicant_years_experience || 0),
      trainingHours: Number(row.applicant_training_hours || 0),
