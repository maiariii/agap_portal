...
ECOSYSTEM_CONFIG = "ecosystem.agap-hrmo.config.cjs"
PM2_NAME     = "agap-hrmo-backend"
...
def main():
    print("\n" + "="*60)
    print("[DEPLOY] AGAP HRMO PORTAL (v1.0)")
    print("="*60)
    ...
    env = os.environ.copy()
    env["VITE_BASE_PATH"] = "/insighted/AGAP-HRMO/"
    env["VITE_API_URL"] = "/insighted/AGAP-HRMO"
    ...
    try:
        ...
        # Transfer via SSH piping to avoid Windows SCP hang bugs
        ssh_pipe_cmd = f'ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 {SSH_KEY_OPT} {REMOTE_USER}@{REMOTE_HOST} "cat > {REMOTE_ROOT}/{ARCHIVE_NAME}"'
        with open(ARCHIVE_NAME, "rb") as f:
            subprocess.run(ssh_pipe_cmd, input=f.read(), shell=True, check=True)
    except subprocess.CalledProcessError:
    ...
    if health.stdout.strip():
        print("      SUCCESS: AGAP HRMO backend confirmed online on port 5055!")
    else:
        print(f"      [WARN] Backend health check failed to respond. Check PM2 logs using 'pm2 logs {PM2_NAME}'.")
    ...
    print("\n" + "="*60)
    print(f"[SUCCESS] AGAP HRMO deployed successfully! (Duration: {duration:.1f}s)")
    print("    URL: https://stride.deped.gov.ph/insighted/AGAP-HRMO/")
    print("="*60 + "\n")
